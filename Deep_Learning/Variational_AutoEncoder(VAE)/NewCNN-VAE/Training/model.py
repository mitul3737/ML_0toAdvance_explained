...
from rnn import MDNRNN, hps_sample, rnn_init_state, rnn_next_state, rnn_output, rnn_output_size
...
def make_model(load_model: bool = True) -> "Model":
  return Model(load_model=load_model)


def clip(x: np.ndarray, lo: float = 0.0, hi: float = 1.0) -> np.ndarray:
...
class Model:
  ...
  def __init__(self, load_model: bool = True) -> None:
    ...
    self.vae = ConvVAE(batch_size=1, gpu_mode=torch.cuda.is_available(), is_training=False, reuse=True, device=device_arg)
    self.rnn = MDNRNN(hps_sample, gpu_mode=torch.cuda.is_available(), reuse=True, device=device_arg)
    if load_model:
  ...
  def encode_obs(self, obs: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    result = np.asarray(obs, dtype=np.float32) / 255.0
  ...
  def get_action(self, z: np.ndarray) -> np.ndarray:
  ...
  def set_model_params(self, model_params: np.ndarray) -> None:
    if EXP_MODE == MODE_Z_HIDDEN:
  ...
  def get_random_model_params(self, stdev: float = 0.1) -> np.ndarray:
  ...
  def init_random_model_params(self, stdev: float = 0.1) -> None:
  ...
  def _load_component_weights(self) -> None:
    vae_path = self._resolve_weight_path("vae")
    rnn_path = self._resolve_weight_path("rnn")
    self.vae.load_json(vae_path)
    self.rnn.load_json(rnn_path)

  @staticmethod
  def _resolve_weight_path(prefix: str) -> str:
    for suffix in (".pt", ".pth", ".json"):
      candidate = Path(f"{prefix}{suffix}")
      if candidate.exists():
        return str(candidate)
    raise FileNotFoundError(f"Unable to locate weights for {prefix}")

# Making a function that runs a 1000-steps simulation of the Full World Model inside the CarRacing environment

def simulate(model: Model, train_mode: bool = False, render_mode: bool = True, num_episode: int = 5, seed: int = -1, max_len: int = -1):
  reward_list = []
  t_list = []
